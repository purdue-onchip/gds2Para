...
int find_Vh_back(fdtdMesh* sys, int sourcePort) {
    ...
    // left hand matrix (D_eps + dt * D_sig + dt ^ 2 * S)
    double* val = (double*)malloc(sys->leng_S * sizeof(double));   // sys->SRowId, sys->SColId, val, COO format of this matrix
    sys->backDiffLeftMatrix(sys->SRowId, sys->SColId, sys->Sval, sys->leng_S, val, dt);
    ...
    for (int ind = 1; ind <= nt; ind++) {    // time marching to find the repeated eigenvectors
        ...
        for (int ind = 0; ind < sys->N_edge - sys->bden; ind++) {
            rsc[ind] = 0;
            rsc[ind] -= sys->stackEpsn[(sys->mapEdgeR[ind] + sys->N_edge_v) / (sys->N_edge_s + sys->N_edge_v)] * EPSILON0 * (-2 * xr[sys->N_edge - sys->bden + ind] + xr[ind]);
        ...
        // backward difference
        status = hypreSolve(sys, sys->SRowId, sys->SColId, val, sys->leng_S, rsc, sys->N_edge - sys->bden, &xr[(sys->N_edge - sys->bden) * 2], 1, 3);
        //status = hypreSolve(sys, LrowId, LcolId, val, leng, rsc, sys->N_edge - sys->bden, &xr[(sys->N_edge - sys->bden) * 2], 0, 3);
        nn = 0;
        for (int ind = 0; ind < sys->N_edge - sys->bden; ind++) {
            nn += pow(xr[(sys->N_edge - sys->bden) * 2 + ind], 2);
            //xr[ind] = xr[sys->N_edge - sys->bden + ind];
            //xr[sys->N_edge - sys->bden + ind] = xr[(sys->N_edge - sys->bden) * 2 + ind];
            //xr[(sys->N_edge - sys->bden) * 2 + ind] = 0;
        }
        nn = sqrt(nn);
